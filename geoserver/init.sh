...
for WF in ${WORKFLOWS}; do
  WS="wf_${WF}"   # each workflow has its own workspace
  LG_NAME="${WF}_tx"

  if ! curl -s -u "$AUTH" "$GEOSERVER_URL/rest/layergroups/${LG_NAME}.xml?workspace=${WS}" \
       | grep -q "<name>${LG_NAME}</name>"; then
    cat > /tmp/${WS}_${LG_NAME}.xml <<EOF
<layerGroup>
  <name>${LG_NAME}</name>
  <mode>SINGLE</mode>
  <layers>
  ...
  <publishables>
    <published type="layer"><name>${WS}:${WF}_tx_flood</name></published>
    <published type="layer"><name>${WS}:${WF}_tx_roads</name></published>
    <published type="layer"><name>${WS}:${WF}_tx_bridges</name></published>
  </publishables>
</layerGroup>
EOF

    curl -s -u "$AUTH" -XPOST -H "Content-type: text/xml" \
      -d @/tmp/${WS}_${LG_NAME}.xml \
      "$GEOSERVER_URL/rest/workspaces/${WS}/layergroups"
  fi
done
...
# Remove the masterpw.info (just a warning helper file)
if [ -f /opt/geoserver/data_dir/security/masterpw.info ]; then
  echo "Removing security/masterpw.info (security risk warning)..."
  rm -f /opt/geoserver/data_dir/security/masterpw.info
fi

# Switch default user/group service to digest encoding (SHA256 if available)
# This overwrites usergroupservice config with digest encoding.
cat > /tmp/usergroup_digest.xml <<EOF
<userGroupService>
  <id>default</id>
  <name>default</name>
  <class>org.geoserver.security.xml.XMLUserGroupService</class>
  <fileName>users.xml</fileName>
  <checkInterval>10</checkInterval>
  <passwordEncoderName>digest</passwordEncoderName>
  <passwordPolicyName>default</passwordPolicyName>
</userGroupService>
EOF

curl -s -u "$AUTH" -XPUT -H "Content-type: application/xml" \
  -d @/tmp/usergroup_digest.xml \
  "$GEOSERVER_URL/rest/security/usergroup/service/default"

# If still using the default admin:geoserver, rotate the password
if curl -s -u admin:geoserver "$GEOSERVER_URL/rest/about/version.json" >/dev/null 2>&1; then
  echo "Default admin password still active, changing it..."
  NEW_ADMIN_PASS="${NEW_ADMIN_PASS:-StrongPWR0cks!}"

  curl -s -u admin:geoserver -XPUT \
    -H "Content-type: application/json" \
    -d "{\"password\":\"${NEW_ADMIN_PASS}\"}" \
    "$GEOSERVER_URL/rest/security/usergroup/service/default/users/admin"

  # Update AUTH so subsequent REST calls use the new password
  AUTH="admin:${NEW_ADMIN_PASS}"
else
  echo "Non-default GEOSERVER_AUTH detected, leaving admin credentials as configured."
fi