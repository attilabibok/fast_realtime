...
def fn_get_geodataframe_from_postgresql(
    str_table_name, dict_db_params, str_geom_col="geometry"
):
    ...
    gdf = gpd.read_postgis(
        f"SELECT * FROM public.{str_table_name}", con=conn, geom_col=str_geom_col
    )
...
def fn_interpolate_wse_from_flow(flow_array, list_rating_curve_str_or_list):
    ...
    # Interpolate
    interpolated_wses = np.interp(
        flow_array, curve_flows, curve_wses, left=np.nan, right=np.nan
    )
...
def fn_create_s_bridge_warning_pnt(cfg: FASTConfig, b_print_output: bool):
    ...
    if b_print_output:
        ...
        logger.info("Step 3: Creating bridge warning points")

    db_params = resolve_db_credentials(cfg.database)

    logger.debug("Computing bridge points")
    df_rating_curves = fn_get_dataframe_from_postgresql(
        "t_bridge_rating_curve", db_params
    )
    ...
    uuid_tuple = (
        tuple(uuid_list) if len(uuid_list) != 1 else (uuid_list[0], uuid_list[0])
    )
    ...
    with psycopg2.connect(**db_params) as conn:
        ...
        """
        gdf_flow_points = gpd.read_postgis(
            sql_query, conn, params=(uuid_tuple,), geom_col="geometry"
        )

    df_rating_max_flow = df_rating_curves.merge(
        df_max_flow, on="nextgen_id", how="left"
    )
    df_rating_max_flow = df_rating_max_flow[
        df_rating_max_flow["max_flow"] >= df_rating_max_flow["min_flow"]
    ]

    if len(df_rating_max_flow) > 0:
        df_rating_max_flow["wse_array"] = df_rating_max_flow.apply(
            lambda row: fn_interpolate_wse_from_flow(
                row["flow_array"], row["list_rating_curve"]
            ),
            axis=1,
        )

        df_max_by_uuid = df_rating_max_flow.groupby("uuid_bridge", as_index=False).agg(
            {"wse_array": fn_max_wse_arrays, "model_run_time": "first"}
        )
        df_max_by_uuid["max_wse"] = df_max_by_uuid["wse_array"].apply(
            lambda x: np.nanmax(x)
        )
        gdf_flow_points = gdf_flow_points.merge(
            df_max_by_uuid, on="uuid_bridge", how="left"
        )
        ...
        gdf_flow_points["is_overtop"] = (
            gdf_flow_points["max_wse"] >= gdf_flow_points["min_overtop"]
        ).astype(int)
        gdf_flow_points["wse_array"] = gdf_flow_points.apply(
            fn_replace_nan_with_min_ground, axis=1
        )
        gdf_flow_points["depth_array"] = gdf_flow_points.apply(
            fn_calculate_depth_array, axis=1
        )
        ...
        gdf_flow_points = gpd.GeoDataFrame(
            columns=[
                "geometry",
                "BRDG_ID",
                "uuid_bridge",
    ...
    with engine.connect() as conn:
        gdf_flow_points.to_postgis(
            "s_bridge_warning_pnt", conn, if_exists="replace", index=False
        )
